;; AUTOMATICALLY GENERATED 2023-12-01 11:12:01.491155 UTC
#lang racket
    ...
    ((list 'tuple-2 'Kt-A (list 'Km-ma1 x-1))
     (if
      ...
      (error
       ...
       x-1)))
    (_ (error 'ERROR "no method implementation for ~a" x-4))))))
(define -star-ty (match-lambda** ((x-4) (match x-4 ('Kt-Any (app '-sig (list))) ('Kt-A 'Kt-A) (_ x-4)))))
(define
 m-ma1-A
 (match-lambda**
  (('tuple-0)
        ...
        (('tuple-0)
         (app
          (app
           ...
           (list
            'tuple-1
            (list 'tuple-3 'Kt-A 'Kt-A (match-lambda** ((x-0) (list 'tuple-3 (app '-sig (list)) x-0 (list)))))))
          'tuple-0))))))))))
(match
 (app
  (app
