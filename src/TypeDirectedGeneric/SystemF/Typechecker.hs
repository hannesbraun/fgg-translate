...
tyOfExp e = do
  case e of
          ...
          if ty2 == tyDom
            then pure tyRes
              ...
              failT
                ( "Function of type "
                    ++ prettyS ty1
                    ++ " applied to value of type "
                    ++ prettyS ty2
                )
        _ ->
          failT
            ( "Left-hand side of function application has not a function type but "
                ++ prettyS ty1
                ++ "\n\n"
                ++ prettyS e
            )
    ...
    ExpPrintf _ _ -> pure $ TyPrim PrimVoid
    ExpSprintf _ _ -> pure $ TyPrim PrimString
  ...
  when (tyScrut /= tyPat) $
    failT
      ( "Type of pattern "
          ...
          ++ " is "
          ++ prettyS tyPat
          ++ ", but this does not match type of scrutinee "
          ++ prettyS tyScrut
      )
