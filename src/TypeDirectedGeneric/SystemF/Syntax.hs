...
data Exp
  ...
  | ExpFail T.Text [Exp]
  | ExpVoid
  | ExpPrintf T.Text [Exp]
  | ExpSprintf T.Text [Exp]
...
test_eqTy = do
  ...
  subAssert $
    check
      True
      (TyForall "a" (TyForall "b" (TyVar "a")))
      (TyForall "b" (TyForall "a" (TyVar "b")))
  subAssert $
    check
      True
      (TyForall "a" (TyForall "b" (TyVar "b")))
      (TyForall "b" (TyForall "a" (TyVar "a")))
  subAssert $
    check
      False
      (TyForall "a" (TyForall "b" (TyVar "a")))
  ...
  subAssert $
    check
      False
      (TyForall "a" (TyForall "b" "a"))
      (TyForall "c" (TyForall "c" "c"))
 where
