...
transProg prog = do
  ...
  return $ Prog
    { p_decls =
          [ DeclClass $ ClassDecl ["r", "f", "t"] [] "HasField" []
               [MeDecl "field" (simpleSigma $ tv "r" `to` tv "f" `to` tv "t")]
          , DeclClass $ ClassDecl ["recv", "name", "args", "res"] [] "Method" []
               [MeDecl "apply" (simpleSigma $
                                    tv "recv" `to` tv "name" `to` tv "args" `to` tv "res")]
          , DeclClass $ ClassDecl ["recv", "name", "res"] [] "InferRes"
               [FunDep ["recv", "name"] "res"]
               [MeDecl "inferRes" (simpleSigma $
...
transIfaceDecl t methods = do
  ...
  (toStructC, toStructI) <- ifCast ([], []) $ pure
      ( [simpleConstr (ClassConstr "ToStruct" [TypeTyVar "a"])]
      , [DeclInst $ InstDecl noInstFlags Set.empty [] "ToStruct" [TypeTyCon tyCon []]
             [MethodDef "toStruct" $
                  ExpLambda "X" (ExpCase (ExpVar "X")
  ...
  return $
      ...
      , DeclInst $ InstDecl noInstFlags (Set.singleton "a") (showC : toStructC ++ pis)
            className [TypeTyVar "a"] []
      , DeclData $ DataDecl tyCon []
            [Constructor k ["b"] [simpleConstr (ClassConstr className [TypeTyVar "b"])]
                 [TypeTyVar "b"] Nothing]
            []
      , DeclInst $ InstDecl noInstFlags Set.empty [] "Show" [simpleTyCon tyCon]
            [MethodDef "show" $
...
transTypeDecl newTy existingTy = do
  ...
  return
      ...
      , DeclInst $ InstDecl noInstFlags Set.empty []
            "Convert" [simpleTyCon (nameTy existingTy), simpleTyCon tyCon]
            [MethodDef "convert" (ExpDataCon dat)]
      , DeclVal $ ValDecl (Just ("Convert a " <> tyConTxt <> " => a -> " <> tyConTxt))
            (nameConvert newTy) [] (var "convert")
      ]
...
transMeDecl ::
    ...
    -> T [Decl]
transMeDecl
  (xg, tg) spec@(G.MeSpec m (G.MeSig args res)) expG = do
...
transIfaceCastDecl goDecls t specs = do
  ...
  where
    makeWitnessCase si exp =
        ExpCase (ExpApp (ExpVar (nameMsv si)) (ExpVar "X"))
...
transMethodCastDecl t spec  =
    let decls =
            [ DeclInst $ InstDecl noInstFlags Set.empty [] (nameMscls spec) [simpleTyCon (nameTy t)]
...
transMethodInstance spec tyCon datCon = do
  ...
  let decl =
          InstDecl noInstFlags tyVars (map simpleConstr ctx) className taus
              [MethodDef "apply" $ ExpLambda "X1" $ ExpLambda "X2" $ ExpLambda "X3" $
                   ExpCase (ExpVar "X1")
                   [( Pat datCon ["X1'"]
                    , var "apply" `app` var "X1'" `app` var "X2" `app` var "X3")]]
  inferInst <-
      transInferRes (simpleTyCon tyCon) (simpleTyCon $ nameMty (G.ms_name spec))
...
transMethodConstraint (G.MeSpec m (G.MeSig args u)) tau = do
  ...
  return $ Constr alphas cs $ ClassConstr "Method"
