...
data Type = Var TyName
          | Tuple [Type]
          | Func Type Type
          | Exists TyName Type
          | Mu Int [TyName] [Type] deriving (Show, Eq) -- starts with 1


data Exp = VarExp VarName
                  -- Haskell backend:
                ...
                | Fold Type Exp
                | Unfold Type Exp deriving Show

-- | Plain projection
mkProj :: (Int, Int) -> Exp -> Exp
mkProj (i,n) e = Proj { projName = Nothing,
                        projIdxs = (i,n),
                        projExps = e }

-- | Plain tuple
mkTuple :: [Exp] -> Exp
mkTuple es = TupleExp { tupleName = Nothing,
                        tupleExps = es }
...
expand (Mu i xs ts)
  | length xs == length ts
    && i >= 0 && i <= length xs = let phi = zip xs ts
                                  in subst phi (ts !! (i-1))
