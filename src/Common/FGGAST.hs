...
data Decl
  = TypeDecl TyName TyFormals TyLit
...
instance TySubstable MeSig where
  applyTySubst subst sig =
    let
      -- Limit the domain of the substitution to variables that
      -- occur free in sig.
      ...
      renaming =
        Map.fromList $
          map
            (findFresh (freeInSubst `Set.union` boundVars) 0)
...
test_tySubstMsig = do
  let subst =
        Map.fromList
          [ ("a", TyNamed "Box" [TyVar "b"]) -- b would be captured
      ...
      sig =
        MeSig
          { msig_tyArgs =
              TyFormals
                [ ("c", Just (TyNamed "EQ" [TyVar "c"]))
                , ("b", Nothing)
                , ("d", Just (TyNamed "I" [TyVar "a"]))
                ]
          , msig_args =
              [ ("x", TyNamed "Box" [TyVar "b"])
