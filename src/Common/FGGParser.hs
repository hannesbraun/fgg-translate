...
test_parseExpr = do
  ...
  subAssert $
    testParse
      "foo.m(Pair(Int,Bool), Bool)(bar, 2)"
...
test_parseProg = do
  subAssert $
    testParse
      "package main func main() { var result = 1 }"
      ( Program
          [anyDecl]
      ...
      , p_mains =
          [ MeBody
              { mb_bindings =
                  ...
                  ,
                    ( VarName{unVarName = "result"}
                    , Nothing
                    , MeCall
                        ( MeCall
                            (Var (VarName{unVarName = "x"}))
                            (MeName{unMeName = "foo"})
                            []
                            []
                        )
  ...
  myProgGeneric =
    Program
      { p_decls =
          ...
          , TypeDecl
              "I1"
              (TyFormals [(TyVarName "a", Nothing)])
              ( Iface
                  [ MeSpec
                      { ms_name = "foo"
                      , ms_sig =
                          MeSig
                            (TyFormals [(TyVarName "c", Nothing)])
                            []
                            (TyNamed "I1" [])
                      }
                  ]
